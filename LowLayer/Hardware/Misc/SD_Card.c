...
{
	while(!__HAL_SPI_GET_FLAG(&qspi6, SPI_FLAG_TXE));

	HAL_SPI_Transmit(&qspi6, &data, 1, 100);
}
...
{
	while(!__HAL_SPI_GET_FLAG(&qspi6, SPI_FLAG_TXE));
	HAL_SPI_Transmit(&qspi6, buffer, len, 100);
}
...
static uint8_t SPI_RxByte(void)
{
	...
	while(!__HAL_SPI_GET_FLAG(&qspi6, SPI_FLAG_TXE));
	HAL_SPI_TransmitReceive(&qspi6, &dummy, &data, 1, 100);
